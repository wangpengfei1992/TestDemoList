...
class ProductViewModel : ViewModel() {
    ...
    fun connectDevice(imp: IBluetoothDevice?, context: Context, productModel: ProductModel, mac: String) {
        launchOnIO {
            val result = imp?.connect(context, productModel, mac)
    ...
    fun insertDefProduct(isBle: Boolean) {
        launchOnIO {
            val result = AnkerWorkDatabase.getDatabase().productDao.insertAll(getDefProduct(isBle))
