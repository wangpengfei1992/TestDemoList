...
object ParseData {
     ...
     */
    @JvmStatic
    fun parseDataToSegment(dataInfo: SppDataInfo): Boolean {
        ...
        while (restLength >= length) {
            ...
            if (mBuff == null) {
//                Log.e(TAG, "mBuff == null");
                break
            }
            restLength = mBuff.size
            if (restLength < 9) {
                Log.e(TAG, "restLength < 9");
                break
            }
...
//            Log.e(TAG, "数据中长度 = $length , 当前长度 = $restLength" )
            if (restLength < length) {
//                Log.e(TAG, "实际数据长度 < 数据中长度，不合法；下一轮校验")
                break
            }
