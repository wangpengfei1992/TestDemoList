...
JNIEXPORT jlong JNICALL Java_com_wpf_opususedemo_utils_OpusUtils_createEncoder
        (JNIEnv *env, jobject thiz, jint sampleRateInHz, jint channelConfig, jint complexity, jint bitRate) {
    int error;
    ...
    if (pOpusEnc) {
        ...
        opus_encoder_ctl(pOpusEnc, OPUS_SET_BITRATE(bitRate));//波特率单位是比特每秒（B / S）
...
JNIEXPORT jlong JNICALL Java_com_wpf_opususedemo_utils_OpusUtils_createDecoder
        (JNIEnv *env, jobject thiz, jint sampleRateInHz, jint channelConfig, jint bitRate) {
    int error;
    OpusDecoder *pOpusDec = opus_decoder_create(sampleRateInHz, channelConfig, &error);
    opus_decoder_ctl(pOpusDec, OPUS_SET_BITRATE(bitRate));
    opus_decoder_ctl(pOpusDec, OPUS_SET_BANDWIDTH(OPUS_BANDWIDTH_WIDEBAND));
    opus_decoder_ctl(pOpusDec, OPUS_SET_VBR(0));
    opus_decoder_ctl(pOpusDec, OPUS_SET_VBR_CONSTRAINT(0));
    opus_decoder_ctl(pOpusDec, OPUS_SET_COMPLEXITY(0));
    opus_decoder_ctl(pOpusDec, OPUS_SET_INBAND_FEC(0));
    opus_decoder_ctl(pOpusDec, OPUS_SET_FORCE_CHANNELS(1));
    opus_decoder_ctl(pOpusDec, OPUS_SET_DTX(0));
    opus_decoder_ctl(pOpusDec, OPUS_SET_PACKET_LOSS_PERC(0));
    opus_decoder_ctl(pOpusDec, OPUS_SET_SIGNAL(OPUS_SIGNAL_VOICE));
    opus_decoder_ctl(pOpusDec, OPUS_SET_EXPERT_FRAME_DURATION(OPUS_FRAMESIZE_20_MS));
    opus_int32 skip;
    opus_decoder_ctl(pOpusDec, OPUS_GET_LOOKAHEAD(&skip));
...
JNIEXPORT jint JNICALL Java_com_wpf_opususedemo_utils_OpusUtils_encode
    ...
    if (nSampleSize - offset < 80 || nByteSize <= 0)
        return 0;
    /*
